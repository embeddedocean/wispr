...
void wispr_print_data_header(wispr_data_header_t *hdr)
{
	...
	fprintf(stdout, "- type: %02x\r\n", hdr->type);
	fprintf(stdout, "- settings: %02x %02x %02x %02x\r\n",hdr->settings[0],hdr->settings[1],hdr->settings[2],hdr->settings[3]);
...
int wispr_parse_config(uint8_t *buf, wispr_config_t *hdr)
{
	...
	hdr->settings[0] = buf[24];
	hdr->settings[1] = buf[25];
	hdr->settings[2] = buf[26];
	hdr->settings[3] = buf[27];
	hdr->settings[4] = buf[28];
	hdr->settings[5] = buf[29];
	hdr->settings[6] = buf[30];
	hdr->settings[7] = buf[31];
...
int wispr_serialize_config(wispr_config_t *hdr, uint8_t *buf)
{
	...
	buf[24] = (uint8_t)(hdr->settings[0]);
	buf[25] = (uint8_t)(hdr->settings[1]);
	buf[26] = (uint8_t)(hdr->settings[2]);
	buf[27] = (uint8_t)(hdr->settings[3]);
	buf[28] = (uint8_t)(hdr->settings[4]);
	buf[29] = (uint8_t)(hdr->settings[5]);
	buf[30] = (uint8_t)(hdr->settings[6]);
	buf[31] = (uint8_t)(hdr->settings[7]);
...
{	
	...
	hdr->settings[0] = wispr->settings[0];
	hdr->settings[1] = wispr->settings[1];
	hdr->settings[2] = wispr->settings[2];
	hdr->settings[3] = wispr->settings[3];
